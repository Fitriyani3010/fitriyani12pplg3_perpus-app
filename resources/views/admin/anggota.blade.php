...
<td class="nunggak">{{ $row->nunggak ?? 0 }}</td>

<td class="denda">Rp {{ number_format($row->total_denda ?? 0) }}</td>
...
<td>
@if($row->status == 'aktif')
<a href="{{ route('admin.anggota.suspend',$row->id_user) }}"
   onclick="return confirm('Hentikan aktivitas siswa ini?')"
   class="action-btn suspend">
   <i class="fas fa-ban"></i>
</a>
@else
<a href="{{ route('admin.anggota.aktifkan',$row->id_user) }}"
